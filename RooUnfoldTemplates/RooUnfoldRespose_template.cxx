...
TVectorD*
RooUnfoldResponse::H2V  (const TH1* h, Int_t nb, Bool_t overflow, std::mutex* myMutex) ////TODO
{
  //! Returns TVectorD of the bin contents of the input histogram
  if (overflow) nb += 2;
  if (myMutex) myMutex->lock (); ////TODO
  TVectorD* v= new TVectorD (nb);
  if (myMutex) myMutex->unlock (); ////TODO
  if (!h) return v;
  for (Int_t i= 0; i < nb; i++) {
...
TH1*
RooUnfoldResponse::ApplyToTruth (const TH1* truth, const char* name, std::mutex* myMutex) const ////TODO
{
