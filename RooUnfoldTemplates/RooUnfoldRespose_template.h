...
public:
  ...
  static TVectorD* H2V  (const TH1*  h, Int_t nb, Bool_t overflow= kFALSE, std::mutex* myMutex=nullptr); ////TODO
  ...
  TH1* ApplyToTruth (const TH1* truth= 0, const char* name= "AppliedResponse", std::mutex* myMutex=nullptr) const; // If argument is 0, applies itself to its own truth ////TODO
