...
//    subpad->SetBorderSize (1);
...

  {
    ...
    for (int i = 1; i < 16; i++) {
      divs->DrawLine (yq[i], ymin, yq[i], h->GetBinContent (h->FindBin (yq[i])));
      tl->DrawLatex (yq[i]+0.16, exp (0.1*log(ymax/ymin)) * ymin, percs[i].Data ());
    }
...
//    subpad->SetBorderSize (1);
...

  {
    ...
    TH2D* h = h2_jet_Pb_fcal_et_zdc_calibE_run312796;
    h->RebinY (4);
    h->GetXaxis ()->SetTitle ("#Sigma#it{E}_{T}^{FCal, Pb} [GeV]");
    h->GetYaxis ()->SetTitle ("Uncalibrated #Sigma#it{E}_{ZDC}^{Pb} [TeV]");
    h->GetZaxis ()->SetTitle ("");
    ...
    myText (0.15, 0.96, kWhite, "Run 312796 (per. A), J50 Trigger", 0.032);

    c->SaveAs ("Plots/ZDCAnalysis/run312796_j50_zdc_fcalet_correlation.pdf");
  }
...

  {
    ...
    TH2D* h = h2_mb_Pb_fcal_et_zdc_calibE_run312796;
    h->RebinY (4);
    ...
    myText (0.15, 0.96, kWhite, "Run 312796 (per. A), MinBias Trigger", 0.032);
   
    c->SaveAs ("Plots/ZDCAnalysis/run312796_mb_zdc_fcalet_correlation.pdf");
  }
}
