...
void CombineFineFCalPtCh (const char* fileTag) {
  ...
  file->Delete ("h_jet_trk_pt_perp_iaa_FineFcalComb;*");
  ...
  h_jet_trk_pt_perp_iaa_comb = (TH1D*) h_jet_trk_pt_perp_iaa[0]->Clone ("h_jet_trk_pt_perp_iaa_FineFcalComb"); 
  h_jet_trk_pt_perp_iaa_comb->Reset ();
  ...
  for (int iCent = 0; iCent < numFineFcalCentBins; iCent++) {
    ...
    h_jet_trk_pt_perp_iaa_comb->Add (h_jet_trk_pt_perp_iaa[iCent], wgt);
  ...
  h_jet_trk_pt_perp_iaa_comb->Scale (1./sum_wgts);
  ...
  h_jet_trk_pt_perp_iaa_comb->Write ();
