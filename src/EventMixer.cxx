#ifndef __EventMixer_cxx__
#define __EventMixer_cxx__
...
bool EventMixer (const char* directory,
                       const int dataSet,
                       const char* inFileName) {
 
  std::cout << "Info: In EventMixer.cxx: Entered EventMixer routine." << std::endl;
  std::cout << "Info: In EventMixer.cxx: Printing systematic onfiguration:";
  std::cout << std::endl;

  if (IsHijing ())
    std::cout << "Info: In EventMixer.cxx: File detected as Hijing overlay" << std::endl;
  if (IsOverlay ())
    std::cout << "Info: In EventMixer.cxx: File detected as data overlay, will check data conditions" << std::endl;
  ...
  std::cout << "Info: In EventMixer.cxx: File Identifier: " << identifier << std::endl;
  std::cout << "Info: In EventMixer.cxx: Saving output to " << rootPath << std::endl;
  ...
  if (TString (inFileName) == "") {
    if (IsCollisions ()) {
      ...
      std::cout << "Error: In EventMixer.C: Cannot identify this MC file! Quitting." << std::endl;
      return false;
    }
  }
  else
  ...
  if (tree == nullptr) {
    std::cout << "Error: In EventMixer.cxx: TTree not obtained for given data set. Quitting." << std::endl;
    return false;
  }
  ...
  TString outTreeName = "";
  if (!Is5TeV ()) {
    std::cout << "Error: In EventMixer.cxx::EventMixer (const char*, const int, const char*): Unsupported beam collision energy, quitting." << std::endl;
    return false;
  }
  else if (IspPb16 ()) {
    outTreeName = "pPbTree";
    ...
    std::cout << "Error: In EventMixer.cxx::EventMixer (const char*, const int, const char*): Beam configuration not recognized, quitting." << std::endl;
    return false;
  }
  ...
  for (int iEvt = 0; iEvt < nEvts; iEvt++) {
    if (nEvts > 100 && iEvt % (nEvts / 100) == 0)
      std::cout << "Info: In EventMixer.cxx: Events " << iEvt / (nEvts / 100) << "\% done...\r" << flush;
    tree->GetEntry (iEvt);
    ...
    if (IspPb ()) {
      ...
      zdc_calibE_Pb = IsPeriodA () ? ZdcRawEnergy_A : ZdcRawEnergy_C; // TODO
      zdc_calibE_p = IsPeriodA () ? ZdcRawEnergy_C : ZdcRawEnergy_A; // TODO
  ...
  std::cout << std::endl << "Info: In EventMixer.cxx: Finished processing events." << std::endl;
