...
void ProcessCorrelations (const char* outFileTag, const char* tag1, const char* tag2 = nullptr) {
  ...
  TH1D**** h_jet_counts_bkg     = Get3DArray <TH1D*> (2, nZdcCentBins+1, nVar);
  ...
  TH1D***** h_jet_trk_pt                = Get4DArray <TH1D*> (2, nDir, nZdcCentBins+1, nVar);
  TH2D***** h2_jet_trk_pt_cov           = Get4DArray <TH2D*> (2, nDir, nZdcCentBins+1, nVar);
  TH1D***** h_jet_trk_dphi              = Get4DArray <TH1D*> (2, nPtChSelections, nZdcCentBins+1, nVar);
  TH2D***** h2_jet_trk_dphi_cov         = Get4DArray <TH2D*> (2, nPtChSelections, nZdcCentBins+1, nVar);

  TH1D***** h_jet_trk_pt_bkg            = Get4DArray <TH1D*> (2, nDir, nZdcCentBins+1, nVar);
  TH2D***** h2_jet_trk_pt_cov_bkg       = Get4DArray <TH2D*> (2, nDir, nZdcCentBins+1, nVar);
  TH1D***** h_jet_trk_dphi_bkg          = Get4DArray <TH1D*> (2, nPtChSelections, nZdcCentBins+1, nVar);
  TH2D***** h2_jet_trk_dphi_cov_bkg     = Get4DArray <TH2D*> (2, nPtChSelections, nZdcCentBins+1, nVar);

  TH1D****  h_jet_trk_pt_ref_sig        = Get3DArray <TH1D*> (2, nDir, nVar);
  TH1D***** h_jet_trk_pt_sig            = Get4DArray <TH1D*> (2, nDir, nZdcCentBins+1, nVar);
  TH1D****  h_jet_trk_dphi_ref_sig      = Get3DArray <TH1D*> (2, nPtChSelections, nVar);
  TH1D***** h_jet_trk_dphi_sig          = Get4DArray <TH1D*> (2, nPtChSelections, nZdcCentBins+1, nVar);

  TH1D**    h_jet_trk_pt_ref_sig_bbb        = Get1DArray <TH1D*> (nDir);
  TH1D***   h_jet_trk_pt_sig_bbb            = Get2DArray <TH1D*> (nDir, nZdcCentBins+1);
  TH1D**    h_jet_trk_dphi_ref_sig_bbb      = Get1DArray <TH1D*> (nPtChSelections);
  TH1D***   h_jet_trk_dphi_sig_bbb          = Get2DArray <TH1D*> (nPtChSelections, nZdcCentBins+1);

  TF1**    f_jet_trk_pt_ref_sig_bbb         = Get1DArray <TF1*> (nDir);
  TF1***   f_jet_trk_pt_sig_bbb             = Get2DArray <TF1*> (nDir, nZdcCentBins+1);
  TF1**    f_jet_trk_dphi_ref_sig_bbb       = Get1DArray <TF1*> (nPtChSelections);
  TF1***   f_jet_trk_dphi_sig_bbb           = Get2DArray <TF1*> (nPtChSelections, nZdcCentBins+1);

  TH1D**    h_jet_trk_pt_ref_sig_closure    = Get1DArray <TH1D*> (nDir);
  TH1D***   h_jet_trk_pt_sig_closure        = Get2DArray <TH1D*> (nDir, nZdcCentBins+1);
  TH1D**    h_jet_trk_dphi_ref_sig_closure  = Get1DArray <TH1D*> (nPtChSelections);
  TH1D***   h_jet_trk_dphi_sig_closure      = Get2DArray <TH1D*> (nPtChSelections, nZdcCentBins+1);

  TF1**    f_jet_trk_pt_ref_sig_closure     = Get1DArray <TF1*> (nDir);
  TF1***   f_jet_trk_pt_sig_closure         = Get2DArray <TF1*> (nDir, nZdcCentBins+1);
  TF1**    f_jet_trk_dphi_ref_sig_closure   = Get1DArray <TF1*> (nPtChSelections);
  TF1***   f_jet_trk_dphi_sig_closure       = Get2DArray <TF1*> (nPtChSelections, nZdcCentBins+1);

  TH1D***** h_jet_trk_pt_iaa                = Get4DArray <TH1D*> (2, nDir, nZdcCentBins+1, nVar);
  TH1D***** h_jet_trk_dphi_iaa              = Get4DArray <TH1D*> (2, nPtChSelections, nZdcCentBins+1, nVar);

  TH1D***   h_jet_trk_pt_iaa_closure        = Get2DArray <TH1D*> (nDir, nZdcCentBins+1);
  TH1D***   h_jet_trk_dphi_iaa_closure      = Get2DArray <TH1D*> (nPtChSelections, nZdcCentBins+1);

  TF1***   f_jet_trk_pt_iaa_closure         = Get2DArray <TF1*> (nDir, nZdcCentBins+1);
  TF1***   f_jet_trk_dphi_iaa_closure       = Get2DArray <TF1*> (nPtChSelections, nZdcCentBins+1);
  ...
  TGAE**** g_jet_trk_dphi_syst          = Get3DArray <TGAE*> (nPtChSelections, nZdcCentBins+1, nVar);
  TGAE**** g_jet_trk_pt_bkg_syst        = Get3DArray <TGAE*> (nDir, nZdcCentBins+1, nVar);
  TGAE**** g_jet_trk_dphi_bkg_syst      = Get3DArray <TGAE*> (nPtChSelections, nZdcCentBins+1, nVar);
  ...
  TGAE**** g_jet_trk_dphi_sig_syst      = Get3DArray <TGAE*> (nPtChSelections, nZdcCentBins+1, nVar);

  TGAE**** g_jet_trk_pt_iaa_syst        = Get3DArray <TGAE*> (nDir, nZdcCentBins+1, nVar);
  TGAE**** g_jet_trk_dphi_iaa_syst      = Get3DArray <TGAE*> (nPtChSelections, nZdcCentBins+1, nVar);
  ...
  TGAE**** g_jet_trk_dphi_systTot         = Get3DArray <TGAE*> (nPtChSelections, nZdcCentBins+1, 3);
  TGAE**** g_jet_trk_pt_bkg_systTot       = Get3DArray <TGAE*> (nDir, nZdcCentBins+1, 3);
  TGAE**** g_jet_trk_dphi_bkg_systTot     = Get3DArray <TGAE*> (nPtChSelections, nZdcCentBins+1, 3);
  ...
  TGAE**** g_jet_trk_dphi_sig_systTot     = Get3DArray <TGAE*> (nPtChSelections, nZdcCentBins+1, 3);

  TGAE**** g_jet_trk_pt_iaa_systTot       = Get3DArray <TGAE*> (nDir, nZdcCentBins+1, 3);
  TGAE**** g_jet_trk_dphi_iaa_systTot     = Get3DArray <TGAE*> (nPtChSelections, nZdcCentBins+1, 3);
    ...
    for (int iVar = 0; iVar < nVar; iVar++) {
...

      {
        ...
        for (int iDir = 0; iDir < nDir; iDir++) {
          ...
          CalcUncertainties (h_jet_trk_pt_ref[iDType][iDir][iVar],   h2_jet_trk_pt_cov_ref[iDType][iDir][iVar],   h_jet_counts_ref[iDType][iVar]);

          h_jet_trk_pt_ref[iDType][iDir][iVar]->Rebin (iDir == 1 ? 5 : 2);
          h_jet_trk_pt_ref[iDType][iDir][iVar]->Scale (1., "width");
        ...
        for (int iPtCh = 0; iPtCh < nPtChSelections; iPtCh++) {
          ...
          CalcUncertainties (h_jet_trk_dphi_ref[iDType][iPtCh][iVar], h2_jet_trk_dphi_cov_ref[iDType][iPtCh][iVar], h_jet_counts_ref[iDType][iVar]);

          h_jet_trk_dphi_ref[iDType][iPtCh][iVar]->Scale (1., "width");
...

      {
        ...
        for (int iDir = 0; iDir < nDir; iDir++) {
          ...
          CalcUncertainties (h_jet_trk_pt_ref_bkg[iDType][iDir][iVar],   h2_jet_trk_pt_cov_ref_bkg[iDType][iDir][iVar],   h_jet_counts_ref_bkg[iDType][iVar]);

          h_jet_trk_pt_ref_bkg[iDType][iDir][iVar]->Rebin (iDir == 1 ? 5 : 2);
          h_jet_trk_pt_ref_bkg[iDType][iDir][iVar]->Scale (1., "width");
        ...
        for (int iPtCh = 0; iPtCh < nPtChSelections; iPtCh++) {
          ...
          CalcUncertainties (h_jet_trk_dphi_ref_bkg[iDType][iPtCh][iVar], h2_jet_trk_dphi_cov_ref_bkg[iDType][iPtCh][iVar], h_jet_counts_ref_bkg[iDType][iVar]);

          h_jet_trk_dphi_ref_bkg[iDType][iPtCh][iVar]->Scale (1., "width");
      ...
      for (int iCent = 0; iCent < nZdcCentBins+1; iCent++) {
        ...
        TString inFileName = Form ("%s/Histograms/%s/JetsHists/%s/%s16_5TeV_%s_hists.root", rootPath.Data (), tag1, var.Data (), dType.Data (), cent.Data ());
        std::cout << "Reading " << inFileName.Data () << std::endl;
        inFile = new TFile (inFileName.Data (), "read");
        outFile->cd ();

        h_evt_counts[iDType][iCent][iVar] = (TH1D*) inFile->Get (Form ("h_evt_counts_%s_%s16", tag1, dType.Data ()))->Clone (Form ("h_evt_counts_pPb_%s_%s_%s", cent.Data (), dType.Data (), var.Data ()));
        h_jet_counts[iDType][iCent][iVar] = (TH1D*) inFile->Get (Form ("h_jet_counts_%s_%s16", tag1, dType.Data ()))->Clone (Form ("h_jet_counts_pPb_%s_%s_%s", cent.Data (), dType.Data (), var.Data ()));

        for (int iDir = 0; iDir < nDir; iDir++) {

          const TString dir = directions[iDir];

          h_jet_trk_pt[iDType][iDir][iCent][iVar]       = (TH1D*) inFile->Get (Form ("h_jet_trk_pt_%s_%s_%s16",       dir.Data (), tag1, dType.Data ()))->Clone (Form ("h_jet_trk_pt_%s_pPb_%s_%s_%s",      dir.Data (), cent.Data (), dType.Data (), var.Data ()));
          h2_jet_trk_pt_cov[iDType][iDir][iCent][iVar]  = (TH2D*) inFile->Get (Form ("h2_jet_trk_pt_%s_cov_%s_%s16",  dir.Data (), tag1, dType.Data ()))->Clone (Form ("h2_jet_trk_pt_%s_cov_pPb_%s_%s_%s", dir.Data (), cent.Data (), dType.Data (), var.Data ()));

          CalcUncertainties (h_jet_trk_pt[iDType][iDir][iCent][iVar],    h2_jet_trk_pt_cov[iDType][iDir][iCent][iVar],    h_jet_counts[iDType][iCent][iVar]);

          h_jet_trk_pt[iDType][iDir][iCent][iVar]->Rebin (iDir == 1 ? 5 : 2);
          h_jet_trk_pt[iDType][iDir][iCent][iVar]->Scale (1., "width");
        ...
        for (int iPtCh = 0; iPtCh < nPtChSelections; iPtCh++) {
          ...
          h_jet_trk_dphi[iDType][iPtCh][iCent][iVar]      = (TH1D*) inFile->Get (Form ("h_jet_trk_dphi_%s_%s_%s16",       ptch.Data (), tag1, dType.Data ()))->Clone (Form ("h_jet_trk_dphi_%s_pPb_%s_%s_%s",       ptch.Data (), cent.Data (), dType.Data (), var.Data ()));
          h2_jet_trk_dphi_cov[iDType][iPtCh][iCent][iVar] = (TH2D*) inFile->Get (Form ("h2_jet_trk_dphi_%s_cov_%s_%s16",  ptch.Data (), tag1, dType.Data ()))->Clone (Form ("h2_jet_trk_dphi_%s_cov_pPb_%s_%s_%s",  ptch.Data (), cent.Data (), dType.Data (), var.Data ()));

          CalcUncertainties (h_jet_trk_dphi[iDType][iPtCh][iCent][iVar], h2_jet_trk_dphi_cov[iDType][iPtCh][iCent][iVar], h_jet_counts[iDType][iCent][iVar]);

          h_jet_trk_dphi[iDType][iPtCh][iCent][iVar]->Scale (1., "width");
      ...
      for (int iCent = 0; iCent < nZdcCentBins+1; iCent++) {
        ...
        TString inFileName = Form ("%s/Histograms/%s/%sHists/%s/%s16_5TeV_%s_hists.root", rootPath.Data (), doMix ? tag1 : tag2, doMix ? "Mixed" : "Jets", var.Data (), dType.Data (), cent.Data ());
        std::cout << "Reading " << inFileName.Data () << std::endl;
        inFile = new TFile (inFileName.Data (), "read");
        outFile->cd ();

        h_evt_counts_bkg[iDType][iCent][iVar] = (TH1D*) inFile->Get (Form ("h_evt_counts_%s_%s16", doMix ? (std::string (tag1) + "_mixed").c_str () : tag2, dType.Data ()))->Clone (Form ("h_evt_counts_pPb_bkg_%s_%s_%s", cent.Data (), dType.Data (), var.Data ()));
        h_jet_counts_bkg[iDType][iCent][iVar] = (TH1D*) inFile->Get (Form ("h_jet_counts_%s_%s16", doMix ? (std::string (tag1) + "_mixed").c_str () : tag2, dType.Data ()))->Clone (Form ("h_jet_counts_pPb_bkg_%s_%s_%s", cent.Data (), dType.Data (), var.Data ()));

        for (int iDir = 0; iDir < nDir; iDir++) {

          const TString dir = directions[iDir];

          h_jet_trk_pt_bkg[iDType][iDir][iCent][iVar]       = (TH1D*) inFile->Get (Form ("h_jet_trk_pt_%s_%s_%s16",       dir.Data (), doMix ? (std::string (tag1) + "_mixed").c_str () : tag2, dType.Data ()))->Clone (Form ("h_jet_trk_pt_%s_pPb_bkg_%s_%s_%s",       dir.Data (), cent.Data (), dType.Data (), var.Data ()));
          h2_jet_trk_pt_cov_bkg[iDType][iDir][iCent][iVar]  = (TH2D*) inFile->Get (Form ("h2_jet_trk_pt_%s_cov_%s_%s16",  dir.Data (), doMix ? (std::string (tag1) + "_mixed").c_str () : tag2, dType.Data ()))->Clone (Form ("h2_jet_trk_pt_%s_cov_pPb_bkg_%s_%s_%s",  dir.Data (), cent.Data (), dType.Data (), var.Data ()));

          CalcUncertainties (h_jet_trk_pt_bkg[iDType][iDir][iCent][iVar],    h2_jet_trk_pt_cov_bkg[iDType][iDir][iCent][iVar],    h_jet_counts_bkg[iDType][iCent][iVar]);

          h_jet_trk_pt_bkg[iDType][iDir][iCent][iVar]->Rebin (iDir == 1 ? 5 : 2);
          h_jet_trk_pt_bkg[iDType][iDir][iCent][iVar]->Scale (1., "width");
        ...
        for (int iPtCh = 0; iPtCh < nPtChSelections; iPtCh++) {
          ...
          h_jet_trk_dphi_bkg[iDType][iPtCh][iCent][iVar]      = (TH1D*) inFile->Get (Form ("h_jet_trk_dphi_%s_%s_%s16",       ptch.Data (), doMix ? (std::string (tag1) + "_mixed").c_str () : tag2, dType.Data ()))->Clone (Form ("h_jet_trk_dphi_%s_pPb_bkg_%s_%s_%s",      ptch.Data (), cent.Data (), dType.Data (), var.Data ()));
          h2_jet_trk_dphi_cov_bkg[iDType][iPtCh][iCent][iVar] = (TH2D*) inFile->Get (Form ("h2_jet_trk_dphi_%s_cov_%s_%s16",  ptch.Data (), doMix ? (std::string (tag1) + "_mixed").c_str () : tag2, dType.Data ()))->Clone (Form ("h2_jet_trk_dphi_%s_cov_pPb_bkg_%s_%s_%s", ptch.Data (), cent.Data (), dType.Data (), var.Data ()));

          CalcUncertainties (h_jet_trk_dphi_bkg[iDType][iPtCh][iCent][iVar], h2_jet_trk_dphi_cov_bkg[iDType][iPtCh][iCent][iVar], h_jet_counts_bkg[iDType][iCent][iVar]);

          h_jet_trk_dphi_bkg[iDType][iPtCh][iCent][iVar]->Scale (1., "width");
  ...
  //////////////////////////////////////////////////////////////////////////////////////////////////// 
  {
    const int iVarNum = GetVarN ("MCTruthJetsTruthParts");
    const int iVarDen = GetVarN ("MCRecoJetTruthParts");
  ...
  for (int iDType = 0; iDType < 2; iDType++) {
    ...
    for (int iVar = 0; iVar < nVar; iVar++) {
      ...
      for (int iDir = 0; iDir < nDir; iDir++) {
        ...
        //if (doUnfold) {// && iDir != 1) {
        //  MultiplyByTF1 (h_jet_trk_pt_ref[iDType][iDir][iVar],     f_jet_trk_pt_ref_sig_bbb[iDir]);
        //  MultiplyByTF1 (h_jet_trk_pt_ref_bkg[iDType][iDir][iVar], f_jet_trk_pt_ref_sig_bbb[iDir]);
        //}
        ...
        for (int iCent = 0; iCent < nZdcCentBins+1; iCent++) {
          ...
          //if (doUnfold) {// && iDir != 1) {
          //  //MultiplyByTF1 (h_jet_trk_pt[iDType][iDir][iCent][iVar],      f_jet_trk_pt_ref_sig_bbb[iDir]);
          //  //MultiplyByTF1 (h_jet_trk_pt_bkg[iDType][iDir][iCent][iVar],  f_jet_trk_pt_ref_sig_bbb[iDir]);
          //  MultiplyByTF1 (h_jet_trk_pt[iDType][iDir][iCent][iVar],      f_jet_trk_pt_sig_bbb[iDir][iCent]);
          //  MultiplyByTF1 (h_jet_trk_pt_bkg[iDType][iDir][iCent][iVar],  f_jet_trk_pt_sig_bbb[iDir][iCent]);
          //}

          h_jet_trk_pt_sig[iDType][iDir][iCent][iVar] = (TH1D*) h_jet_trk_pt[iDType][iDir][iCent][iVar]->Clone (Form ("h_jet_trk_pt_%s_pPb_sig_%s_%s_%s", dir.Data (), cent.Data (), dType.Data (), var.Data ()));
          if (hasBkg)
          ...
          h_jet_trk_pt_iaa[iDType][iDir][iCent][iVar] = (TH1D*) h_jet_trk_pt_sig[iDType][iDir][iCent][iVar]->Clone (Form ("h_jet_trk_pt_%s_iaa_%s_%s_%s", dir.Data (), cent.Data (), dType.Data (), var.Data ()));
      ...
      for (int iPtCh = 0; iPtCh < nPtChSelections; iPtCh++) {
        ...
        for (int iCent = 0; iCent < nZdcCentBins+1; iCent++) {
          ...
          h_jet_trk_dphi_sig[iDType][iPtCh][iCent][iVar] = (TH1D*) h_jet_trk_dphi[iDType][iPtCh][iCent][iVar]->Clone (Form ("h_jet_trk_dphi_%s_pPb_sig_%s_%s_%s", ptch.Data (), cent.Data (), dType.Data (), var.Data ()));
          if (hasBkg)
            h_jet_trk_dphi_sig[iDType][iPtCh][iCent][iVar]->Add (h_jet_trk_dphi_bkg[iDType][iPtCh][iCent][iVar], -1);

          h_jet_trk_dphi_iaa[iDType][iPtCh][iCent][iVar] = (TH1D*) h_jet_trk_dphi_sig[iDType][iPtCh][iCent][iVar]->Clone (Form ("h_jet_trk_dphi_%s_iaa_%s_%s_%s", ptch.Data (), cent.Data (), dType.Data (), var.Data ()));
  ...
  //////////////////////////////////////////////////////////////////////////////////////////////////// 
  {
    const int iVarNum = 0;//GetVarN ("Nominal");
    const int iVarDen = GetVarN ("MCTruthJetsTruthParts");
    ...
    for (int iDir = 0; iDir < nDir; iDir++) {
      ...
      h = (TH1D*) h_jet_trk_pt_ref_sig[1][iDir][iVarNum]->Clone (Form ("h_jet_trk_pt_%s_ref_sig_closure", dir.Data ()));
      h->Divide (h_jet_trk_pt_ref_sig[1][iDir][iVarDen]);
      h_jet_trk_pt_ref_sig_closure[iDir] = h;

      f = new TF1 (Form ("f_jet_trk_pt_%s_ref_sig_closure", dir.Data ()), funcStr, h->GetXaxis ()->GetBinLowEdge (1), h->GetXaxis ()->GetBinLowEdge (h->GetNbinsX () + 1));
      //f = new TF1 (Form ("f_jet_trk_pt_%s_ref_sig_closure", dir.Data ()), &bbbf, h->GetXaxis ()->GetBinLowEdge (1), h->GetXaxis ()->GetBinLowEdge (h->GetNbinsX () + 1), ndf);
      if (iDir == 1) {
        double x1, x2;
        f->GetRange (x1, x2);
        f->SetRange (x1, 20);
      ...
      f_jet_trk_pt_ref_sig_closure[iDir] = f;
      h->Fit (f, "RN0Q");
      ...
      for (int iCent = 0; iCent < nZdcCentBins+1; iCent++) {
        ...
        h = (TH1D*) h_jet_trk_pt_sig[1][iDir][iCent][iVarNum]->Clone (Form ("h_jet_trk_pt_%s_pPb_sig_closure_%s", dir.Data (), cent.Data ()));
        h->Divide (h_jet_trk_pt_sig[1][iDir][iCent][iVarDen]);
        h_jet_trk_pt_sig_closure[iDir][iCent] = h;

        f = new TF1 (Form ("f_jet_trk_pt_%s_pPb_sig_closure_%s", dir.Data (), cent.Data ()), funcStr, h->GetXaxis ()->GetBinLowEdge (1), h->GetXaxis ()->GetBinLowEdge (h->GetNbinsX () + 1));
        //f = new TF1 (Form ("f_jet_trk_pt_%s_pPb_closure_%s", dir.Data (), cent.Data ()), &bbbf, h->GetXaxis ()->GetBinLowEdge (1), h->GetXaxis ()->GetBinLowEdge (h->GetNbinsX () + 1), ndf);
        if (iDir == 1) {
          double x1, x2;
          f->GetRange (x1, x2);
          f->SetRange (x1, 20);
        ...
        f_jet_trk_pt_sig_closure[iDir][iCent] = f;
        h->Fit (f, "RN0Q");


        h = (TH1D*) h_jet_trk_pt_iaa[1][iDir][iCent][iVarNum]->Clone (Form ("h_jet_trk_pt_%s_iaa_closure_%s", dir.Data (), cent.Data ()));
        h->Divide (h_jet_trk_pt_iaa[1][iDir][iCent][iVarDen]);
        h_jet_trk_pt_iaa_closure[iDir][iCent] = h;

        f = new TF1 (Form ("f_jet_trk_pt_%s_iaa_closure_%s", dir.Data (), cent.Data ()), funcStr, h->GetXaxis ()->GetBinLowEdge (1), h->GetXaxis ()->GetBinLowEdge (h->GetNbinsX () + 1));
        //f = new TF1 (Form ("f_jet_trk_pt_%s_iaa_closure_%s", dir.Data (), cent.Data ()), &bbbf, h->GetXaxis ()->GetBinLowEdge (1), h->GetXaxis ()->GetBinLowEdge (h->GetNbinsX () + 1), ndf);
        if (iDir == 1) {
          double x1, x2;
          f->GetRange (x1, x2);
          f->SetRange (x1, 20);
        ...
        f_jet_trk_pt_iaa_closure[iDir][iCent] = f;
        h->Fit (f, "RN0Q");
  ...
  for (int iDType = 0; iDType < 2; iDType++) {
    ...
    for (int iVar = 0; iVar < nVar; iVar++) {
      ...
      const float scaleFactor = (var == "NonClosureVar" ? 0.5 : 1.); // for non-closure systematic scale correction by only 50%

      for (int iDir = 0; iDir < nDir; iDir++) {

        DivideByTF1 (h_jet_trk_pt_ref_sig[iDType][iDir][iVar], f_jet_trk_pt_ref_sig_closure[iDir], scaleFactor);

        for (int iCent = 0; iCent < nZdcCentBins+1; iCent++) {

          DivideByTF1 (h_jet_trk_pt_sig[iDType][iDir][iCent][iVar], f_jet_trk_pt_sig_closure[iDir][iCent], scaleFactor);
          DivideByTF1 (h_jet_trk_pt_iaa[iDType][iDir][iCent][iVar], f_jet_trk_pt_iaa_closure[iDir][iCent], scaleFactor);
  ...
  //////////////////////////////////////////////////////////////////////////////////////////////////// 
  {
    ...
    for (int iVar = 0; iVar < nVar; iVar++) {
      ...
      for (int iDir = 0; iDir < nDir; iDir++) {
        ...
        for (int iCent = 0; iCent < nZdcCentBins+1; iCent++) {
          ...
          g_jet_trk_pt_syst[iDir][iCent][iVar]->Write      (Form ("g_jet_trk_pt_%s_syst_%s_%s",     dir.Data (), cent.Data (), var.Data ()));
          g_jet_trk_pt_bkg_syst[iDir][iCent][iVar]->Write  (Form ("g_jet_trk_pt_%s_bkg_syst_%s_%s", dir.Data (), cent.Data (), var.Data ()));
          g_jet_trk_pt_sig_syst[iDir][iCent][iVar]->Write  (Form ("g_jet_trk_pt_%s_sig_syst_%s_%s", dir.Data (), cent.Data (), var.Data ()));
          g_jet_trk_pt_iaa_syst[iDir][iCent][iVar]->Write  (Form ("g_jet_trk_pt_%s_iaa_syst_%s_%s", dir.Data (), cent.Data (), var.Data ()));
      ...
      for (int iPtCh = 0; iPtCh < nPtChSelections; iPtCh++) {
        ...
        for (int iCent = 0; iCent < nZdcCentBins+1; iCent++) {
          ...
          g_jet_trk_dphi_syst[iPtCh][iCent][iVar]->Write     (Form ("g_jet_trk_dphi_%s_syst_%s_%s",     ptch.Data (), cent.Data (), var.Data ()));
          g_jet_trk_dphi_bkg_syst[iPtCh][iCent][iVar]->Write (Form ("g_jet_trk_dphi_%s_bkg_syst_%s_%s", ptch.Data (), cent.Data (), var.Data ()));
          g_jet_trk_dphi_sig_syst[iPtCh][iCent][iVar]->Write (Form ("g_jet_trk_dphi_%s_sig_syst_%s_%s", ptch.Data (), cent.Data (), var.Data ()));
          g_jet_trk_dphi_iaa_syst[iPtCh][iCent][iVar]->Write (Form ("g_jet_trk_dphi_%s_iaa_syst_%s_%s", ptch.Data (), cent.Data (), var.Data ()));
    ...
    for (int iTotVar = 0; iTotVar < 3; iTotVar++) {
      ...
      for (int iDir = 0; iDir < nDir; iDir++) {
        ...
        for (int iCent = 0; iCent < nZdcCentBins+1; iCent++) {
          ...
          g_jet_trk_pt_systTot[iDir][iCent][iTotVar]->Write      (Form ("g_jet_trk_pt_%s_%s_systTot_%s",     dir.Data (), totVar.Data (), cent.Data ()));
          g_jet_trk_pt_bkg_systTot[iDir][iCent][iTotVar]->Write  (Form ("g_jet_trk_pt_%s_bkg_%s_systTot_%s", dir.Data (), totVar.Data (), cent.Data ()));
          g_jet_trk_pt_sig_systTot[iDir][iCent][iTotVar]->Write  (Form ("g_jet_trk_pt_%s_sig_%s_systTot_%s", dir.Data (), totVar.Data (), cent.Data ()));
          g_jet_trk_pt_iaa_systTot[iDir][iCent][iTotVar]->Write  (Form ("g_jet_trk_pt_%s_iaa_%s_systTot_%s", dir.Data (), totVar.Data (), cent.Data ()));
      ...
      for (int iPtCh = 0; iPtCh < nPtChSelections; iPtCh++) {
        ...
        for (int iCent = 0; iCent < nZdcCentBins+1; iCent++) {
          ...
          g_jet_trk_dphi_systTot[iPtCh][iCent][iTotVar]->Write     (Form ("g_jet_trk_dphi_%s_%s_systTot_%s",      ptch.Data (), totVar.Data (), cent.Data ()));
          g_jet_trk_dphi_bkg_systTot[iPtCh][iCent][iTotVar]->Write (Form ("g_jet_trk_dphi_%s_bkg_%s_systTot_%s",  ptch.Data (), totVar.Data (), cent.Data ()));
          g_jet_trk_dphi_sig_systTot[iPtCh][iCent][iTotVar]->Write (Form ("g_jet_trk_dphi_%s_sig_%s_systTot_%s",  ptch.Data (), totVar.Data (), cent.Data ()));
          g_jet_trk_dphi_iaa_systTot[iPtCh][iCent][iTotVar]->Write (Form ("g_jet_trk_dphi_%s_iaa_%s_systTot_%s",  ptch.Data (), totVar.Data (), cent.Data ()));
    ...
    for (int iDir = 0; iDir < nDir; iDir++) {
      ...
      h_jet_trk_pt_ref_sig_bbb[iDir]->Write ();
      f_jet_trk_pt_ref_sig_bbb[iDir]->Write ();

      h_jet_trk_pt_ref_sig_closure[iDir]->Write ();
      f_jet_trk_pt_ref_sig_closure[iDir]->Write ();

      for (int iCent = 0; iCent < nZdcCentBins+1; iCent++) {

        h_jet_trk_pt_sig_bbb[iDir][iCent]->Write ();
        f_jet_trk_pt_sig_bbb[iDir][iCent]->Write ();

        h_jet_trk_pt_sig_closure[iDir][iCent]->Write ();
        f_jet_trk_pt_sig_closure[iDir][iCent]->Write ();

        h_jet_trk_pt_iaa_closure[iDir][iCent]->Write ();
        f_jet_trk_pt_iaa_closure[iDir][iCent]->Write ();
