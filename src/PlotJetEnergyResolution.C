...
void PlotJetEnergyResolution () {
  ...
  TFile* inFile = new TFile (Form ("%s/rootFiles/JetEnergyResolution/Nominal/summary.root", workPath.Data ()), "read");
  ...
  for (int iSys : systems) {
    ...
    h2_avg_jpts[iSys] = (TH2D*) inFile->Get (Form ("h2_avg_jpts_%s", sys.Data ()));
    h2_avg_jptr[iSys] = (TH2D*) inFile->Get (Form ("h2_avg_jptr_%s", sys.Data ()));

    h2_avg_jes[iSys] = (TH2D*) inFile->Get (Form ("h2_avg_jes_%s", sys.Data ()));
    h2_avg_jer[iSys] = (TH2D*) inFile->Get (Form ("h2_avg_jer_%s", sys.Data ()));

    h2_avg_jetacorr[iSys] = (TH2D*) inFile->Get (Form ("h2_avg_jetacorr_%s", sys.Data ()));
    h2_avg_jetares[iSys] = (TH2D*) inFile->Get (Form ("h2_avg_jetares_%s", sys.Data ()));
    ...
    for (int iEta = 0; iEta <= numEtaBins; iEta++) {
      ...
      h_avg_jpts[iSys][iEta] = (TH1D*) inFile->Get (Form ("h_avg_jpts_%s_iEta%i", sys.Data (), iEta));
      h_avg_jptr[iSys][iEta] = (TH1D*) inFile->Get (Form ("h_avg_jptr_%s_iEta%i", sys.Data (), iEta));

      h_avg_jes[iSys][iEta] = (TH1D*) inFile->Get (Form ("h_avg_jes_%s_iEta%i", sys.Data (), iEta));
      h_avg_jer[iSys][iEta] = (TH1D*) inFile->Get (Form ("h_avg_jer_%s_iEta%i", sys.Data (), iEta));

      h_avg_jetacorr[iSys][iEta] = (TH1D*) inFile->Get (Form ("h_avg_jetacorr_%s_iEta%i", sys.Data (), iEta));
      h_avg_jetares[iSys][iEta] = (TH1D*) inFile->Get (Form ("h_avg_jetares_%s_iEta%i", sys.Data (), iEta));
  ...
  for (int iSys : systems) {
    ...
    TCanvas* c = new TCanvas (Form ("c_jpts_%s", sys.Data ()), "", 800, 800);
...

    {
      ...
      TH1D* htemp = new TH1D ("htemp", "", 1, enJBins[0], enJBins[numEnJBins]);
      ...
      l->DrawLine (enJBins[0], 100, enJBins[numEnJBins], 100);
    }
     

    for (int iEta = 0; iEta < numEtaBins; iEta++) {
      TGAE* g = make_graph (h_avg_jpts[iSys][iEta]);
    ...
    {
      TGAE* g = make_graph (h_avg_jpts[iSys][numEtaBins]);
    ...
    c->SaveAs (Form ("%s/Plots/JetEnergyResolution/JPTS_%s.pdf", workPath.Data (), sys.Data ()));
  } // end loop over iSys
  ...
  for (int iSys : systems) {
    ...
    TCanvas* c = new TCanvas (Form ("c_jes_%s", sys.Data ()), "", 800, 800);
    const double lMargin = 0.15;
    const double rMargin = 0.04;
    const double bMargin = 0.15;
    const double tMargin = 0.04;
    ...
    for (int iEta = 0; iEta < numEtaBins; iEta++) {
      TGAE* g = make_graph (h_avg_jes[iSys][iEta]);
    ...
    {
      TGAE* g = make_graph (h_avg_jes[iSys][numEtaBins]);
    ...
    c->SaveAs (Form ("%s/Plots/JetEnergyResolution/JES_%s.pdf", workPath.Data (), sys.Data ()));
  } // end loop over iSys
  ...
  for (int iSys : systems) {
    ...
    TCanvas* c = new TCanvas (Form ("c_jptr_%s", sys.Data ()), "", 800, 800);
    const double lMargin = 0.15;
    const double rMargin = 0.04;
    const double bMargin = 0.15;
    const double tMargin = 0.04;
    ...
    for (int iEta = 0; iEta < numEtaBins; iEta++) {
      TGAE* g = make_graph (h_avg_jptr[iSys][iEta]);
    ...
    {
      TGAE* g = make_graph (h_avg_jptr[iSys][numEtaBins]);
    ...
    c->SaveAs (Form ("%s/Plots/JetEnergyResolution/JPTR_%s.pdf", workPath.Data (), sys.Data ()));
  }
  ...
  for (int iSys : systems) {
    ...
    TCanvas* c = new TCanvas (Form ("c_jer_%s", sys.Data ()), "", 800, 800);
    const double lMargin = 0.15;
    const double rMargin = 0.04;
    const double bMargin = 0.15;
    const double tMargin = 0.04;
    ...
    for (int iEta = 0; iEta < numEtaBins; iEta++) {
      TGAE* g = make_graph (h_avg_jer[iSys][iEta]);
    ...
    {
      TGAE* g = make_graph (h_avg_jer[iSys][numEtaBins]);
    ...
    c->SaveAs (Form ("%s/Plots/JetEnergyResolution/JER_%s.pdf", workPath.Data (), sys.Data ()));
  }
